...
const messages = {
  startingPosition: {
    ...
    invalid: startingPosition => {
      console.log(
        `${CGS.red(
          `Please provide only 2 numbers followed a single letter ['N','E','S','W'] separated by spaces: ${startingPosition}`
        )}`
      );
    },
    leavingGrid: () => {
      console.log(
        `${CGS.red(
          'Starting position provided is out of bounds of grid, please enter a new one.'
        )}`
      );
    },
    invalidOrientation: () => {
      console.log(
        `${CGS.red(
          "Orientation must be one of 4 possible inputs ['N','E','S','W']"
        )}`
      );
    }
  },
  gridSize: {
    tooManyArguments: (gridSize: string) => {
      console.log(
        `${CGS.red(
          `Too many arguments, please provide only 2 numbers separated by a space: ${gridSize}`
        )}`
      );
    },
    ...
    largeArea: gridSize =>
      readlineSync.question(
        'Some features are limited to a smaller grid sizes than what was enter, but you may still continue, would you like to proceed with your entry? [Y]es [N]o'
      )
  }
};

export const validateStartingPosition = (
